...
const PostureMonitoringScreen = ({navigation, route}) => {
  ...
  const webViewSource = Platform.select({
    ...
    ios: require('../../ios/assets/interview.html'),
  });
  ...
  const isBackground = route?.params?.setBackground ?? false;
  ...
  const [backgroundMode, setBackgroundMode] = useState(isBackground);
  ...
  const handleMessage = useCallback(async event => {
    try {
      console.log('📥 Raw WebView message data is :', event.nativeEvent.data);
      ...
      // } catch (e) {
      //   console.error(
      //     '[WebView Error] ❌ Error parsing message:',
      //     event.nativeEvent.data,
      //   );
      //   setWebViewError('❌ Error parsing message: ' + event.nativeEvent.data);
      //   return;
      // }
      ...
      switch (data.type) {
        ...
        case 'grooming_analysis':
          console.log('grooming messageis thiiiiiiis ', data.grooming.result.attireType);
          setGroomingData(data.grooming.result.attireType)
        ...
        case 'background_analysis':
          // Process pose data with timestamp
          console.log('background mode chal geya', data.background.result.attireType);
          setBackgroundData(data.background.result.attireType);
          break;

        case 'attire_analysis':
          setAttireData(data.attire.result.attireType);
          console.log('attire mode chal geya', data.attire.result.attireType);
          break;
  ...
  const captureAndSendPhoto = async () => {
    ...
    console.log('⚠️ Skipping photo capture: already processing');
    return;
  }
  processingRef.current = true;
    ...
    try {
      ...
      if (backgroundMode === true) {
        console.log('background check modal is running');
        const message = {
          command: 'background',
          image: `data:image/jpeg;base64,${base64}`,
          from: 'sending photo',
  ...
  useEffect(() => {
    ...
    if (isMonitoring && webViewLoaded && cameraRef.current) {
      ...
      if (!processingRef.current) {
        ...
        console.log('⏳ Waiting... previous frame still processing');
      }
    }, 3000);
